...
FASTAQUAL_SELECT   = "python -m metawrap2.scripts.blobology.fastaqual_select -f {assembly} -s r -n {n_contigs}"
BLASTN             = ("blastn -task megablast -query {query} -db {blastdb}/nt -evalue 1e-5"
...
GC_COV_ANNOTATE    = ("python -m metawrap2.scripts.blobology.gc_cov_annotate --blasttaxid {megablast}"
                      " --assembly {assembly} --bam {bams} --out {out} --taxdump {taxdump}"
                      " --taxlist species genus family order subclass phylum superkingdom")
ADD_BINS           = "python -m metawrap2.scripts.blobology.add_bins_to_blobplot {blobplot} {bin_folder}"
MAKEBLOBPLOT       = "python -m metawrap2.scripts.blobology.makeblobplot {table} {prop} {taxlevel}"
MAKEBLOBPLOT_BASE  = "python -m metawrap2.scripts.blobology.makeblobplot {table} {prop} {taxlevel} {base}"
# ─── CONSTANTS ───────────────────────────────────────────────────────────────────────────
...
def _bams(out: str) -> str:
    return " ".join(sorted(glob.glob(os.path.join(out, "*.bowtie2.bam"))))
...
def _annotate_with_bins(blobplot: str, binned_blobplot: str, bin_folder: str) -> bool:
    ...
    run(ADD_BINS.format(blobplot=blobplot, bin_folder=bin_folder),
        env=None, tool="add_bins_to_blobplot", log_path=tmp)
    with open(tmp) as fh:
        binned = [ln for ln in fh if "Unbinned" not in ln]
    if len(binned) < 2:
        warning("No contig matches were found in the bins provided. This may be because the "
...
def _plot(table: str, prop: float, taxlevel: str, env: Optional[str], base: Optional[str] = None) -> None:
    if base is None:
    ...
    run(cmd, env=env, tool="makeblobplot")
...
def main(argv: List[str]) -> int:
    ...
    rec = start_run("blobology", args, env, settings, inputs=[args.assembly, args.bins] + list(args.reads))
    ...
    try:
        ...
        if not os.path.isfile(os.path.join(blastdb, "nt.00.nhd")):
            error("The file %s/nt.00.nhd does not exist, which likely means the NCBI nt database "
                  "path (BLASTDB) is not set correctly or the database is not downloaded." % blastdb)
        if not os.path.isfile(os.path.join(taxdump, "citations.dmp")):
            error("The file %s/citations.dmp does not exist, which likely means the NCBI taxonomy "
        ...
        if ckpt.todo("megablast"):
            ...
            if args.subsample is None:
                ...
                run(FASTAQUAL_SELECT.format(assembly=args.assembly, n_contigs=args.subsample),
                    env=env, tool="fastaqual_select", log_path=assembly_copy)

            comm("Running MEGABLAST on %s" % assembly)
            if not (os.path.isfile(megablast) and os.path.getsize(megablast)):
                run(BLASTN.format(query=assembly_copy, blastdb=blastdb, threads=args.threads),
                    env=env, tool="blastn", log_path=megablast)
            else:
        ...
        if ckpt.todo("map_reads"):
            ...
            if not os.path.isfile(assembly_copy + ".1.bt2"):
                comm("Indexing %s" % assembly_copy)
                run(BOWTIE2_BUILD.format(threads=args.threads, assembly=assembly_copy, index=assembly_copy),
                    env=env, tool="bowtie2-build")
            else:
                comm("Looks like the assembly was already indexed. Skipping...")

            for sname, r1, r2 in pairs:
                bam = os.path.join(out, sname + ".bowtie2.bam")
                if os.path.isfile(bam) and os.path.getsize(bam):
                    comm("Looks like the alignment file for %s already exists. Skipping..." % sname)
                    continue
                sam = os.path.join(out, sname + ".bowtie2.sam")
                comm("Now processing sample %s ... Aligning %s and %s to %s with bowtie2"
                     % (sname, r1, r2, assembly_copy))
                run(BOWTIE2.format(index=assembly_copy, threads=args.threads, r1=r1, r2=r2),
                    env=env, tool="bowtie2", log_path=sam)
                comm("converting %s alignment to bam format" % sam)
                run(SAMTOOLS_VIEW.format(threads=args.threads, sam=sam), env=env,
                    tool="samtools view", log_path=bam)
            ckpt.done("map_reads")
        else:
        ...
        if ckpt.todo("blobfile"):
            ...
            if not (os.path.isfile(blobplot) and os.path.getsize(blobplot)):
                run(GC_COV_ANNOTATE.format(megablast=megablast, assembly=assembly_copy, bams=_bams(out),
                                           out=blobplot, taxdump=taxdump), env=env, tool="gc_cov_annotate")
                if not (os.path.isfile(blobplot) and os.path.getsize(blobplot)):
                    error("Something went wrong with making the blob file from the .bam and .megablast "
                          "files. Exiting.")
            ...
            if args.bins:
                ...
                have_bins = _annotate_with_bins(blobplot, binned_blobplot, args.bins)
            ckpt.done("blobfile")
        else:
            comm("skipping blob file creation (already done; --resume)")
            have_bins = bool(args.bins) and os.path.isfile(binned_blobplot) \
        ...
        if ckpt.todo("plots"):
            ...
            if not os.path.isfile(blobplot + ".taxlevel_phylum.png"):
            ...
            if have_bins:
                ...
                comm("making blobplots of only binned contigs with phylogeny annotations "
        ...
        comm("cleaning up...")
        for pattern in ("*.bowtie2.sam", "*.bowtie2.bam"):
