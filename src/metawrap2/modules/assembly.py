...
METASPADES_RESTART = "metaspades.py -o {out} --restart-from last -t {threads} -m {mem} --tmp-dir {tmp}"
...
def _parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    p.add_argument("-m", "--memory", type=int, default=DEFAULT_MEM_GB, help="memory in GB (default 24)")
    ...
    p.add_argument("--metaspades", action="store_true",
...
def _run_metaspades(args, env, spades_out):
    announcement("ASSEMBLING WITH METASPADES")
    comm("Using reads %s and %s for assembly," % (args.reads_1, args.reads_2))
    tmp = os.path.join(args.output, "metaspades.tmp")
    ensure_dir(tmp)
    if os.path.isfile(os.path.join(spades_out, "spades.log")):
        run(METASPADES_RESTART.format(out=spades_out, threads=args.threads, mem=args.memory, tmp=tmp),
            env=env, tool="metaspades.py")
    else:
        run(METASPADES.format(tmp=tmp, threads=args.threads, mem=args.memory, out=spades_out,
                              r1=args.reads_1, r2=args.reads_2),
            env=env, tool="metaspades.py")
    if not os.path.isfile(os.path.join(spades_out, "scaffolds.fasta")):
        error("Something went wrong with metaSPAdes assembly. Exiting.")
...
def _extract_unmapped(args, env, spades_out):
    ...
    if not os.path.getsize(long_scaffolds):
        error("metaSPAdes produced no contigs over 1.5kb, so the rest of the pipeline wont work. Exiting.")
    ...
    run(BWA_MEM.format(threads=args.threads, assembly=long_scaffolds, r1=args.reads_1, r2=args.reads_2),
        env=env, tool="bwa mem", log_path=aln_sam)
...
def _run_megahit(args, env, megahit_out, spades_out, hybrid):
    announcement("ASSEMBLING READS WITH MEGAHIT")
    tmp = os.path.join(args.output, "megahit.tmp")
    ensure_dir(tmp)
    mem = args.memory * MEGAHIT_MEM_BYTES
    if hybrid:
        unused_fastq = os.path.join(args.output, "unused_by_metaspades.fastq")
        comm("assembling %s with megahit" % unused_fastq)
        run(MEGAHIT_LEFTOVERS.format(reads=unused_fastq, out=megahit_out, threads=args.threads,
                                     mem=mem, tmp=tmp),
            env=env, tool="megahit")
        os.replace(unused_fastq, os.path.join(spades_out, "unused_by_metaspades.fastq"))
    else:
        comm("assembling %s and %s with megahit" % (args.reads_1, args.reads_2))
        run(MEGAHIT_PAIRED.format(r1=args.reads_1, r2=args.reads_2, out=megahit_out, tmp=tmp,
                                  threads=args.threads, mem=mem),
            env=env, tool="megahit")
    if not os.path.isfile(os.path.join(megahit_out, "final.contigs.fa")):
        error("Something went wrong with reassembling with Megahit. Exiting.")
    shutil.rmtree(tmp, ignore_errors=True)
...
def _format_assembly(args, megahit_out, spades_out, do_megahit, do_metaspades):
    ...
    if do_megahit and do_metaspades:
        ...
        with open(long_contigs, "w") as out:
            fix_megahit_contig_naming.fix_naming(os.path.join(megahit_out, "final.contigs.fa"),
                                                 args.min_len, out)
        ...
        with open(final_assembly, "w") as out:
            rm_short_contigs.remove_short_contigs(args.min_len,
                                                  os.path.join(spades_out, "scaffolds.fasta"), out)
...
def main(argv: List[str]) -> int:
    ...
    if not os.path.isfile(args.reads_1) or not os.path.isfile(args.reads_2):
        error("Read files %s and/or %s dont exist. Exiting." % (args.reads_1, args.reads_2))
    ...
    try:
        ...
        if do_metaspades and ckpt.todo("metaspades"):
            _run_metaspades(args, env, spades_out)
        ...
        if do_megahit and ckpt.todo("megahit"):
            _run_megahit(args, env, megahit_out, spades_out, hybrid)
            ckpt.done("megahit")
        ...
        if ckpt.todo("quast"):
            ...
            run(QUAST.format(threads=args.threads, out=quast_out, quast_min=QUAST_MIN_CONTIG,
