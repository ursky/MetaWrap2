...
def test_command_templates_format_cleanly():
    cmd = read_qc.FASTQC.format(threads=4, outdir="out/pre", r1="a_1.fastq", r2="a_2.fastq")
    assert "{" not in cmd and "fastqc -q -t 4" in cmd

    cmd = read_qc.TRIM_GALORE.format(outdir="out", r1="a_1.fastq", r2="a_2.fastq")
    assert "{" not in cmd and "--paired" in cmd

    cmd = read_qc.BMTAGGER.format(bitmask="db/hg38.bitmask", srprism="db/hg38.srprism",
                                  tmpdir="out/tmp", r1="a_1.fastq", r2="a_2.fastq",
