...
Before you start, install the MetaWrap2 core (`pip install -e .`), create the conda environment(s) for
the modules you plan to run (`metawrap2 install-env <module>` or `--all`), and set your database paths
in `~/.metawrap2/config.toml`. See the [installation guide](installation/metawrap_installation.md) and
[database guide](installation/database_installation.md). You can confirm a module's tools and databases
are ready before running it with `metawrap2 check <module>`.

Every module is invoked as `metawrap2 <module> ...` and prints its own help with `metawrap2 <module> -h`.
...
```
metawrap2 assembly -1 CLEAN_READS/ALL_READS_1.fastq -2 CLEAN_READS/ALL_READS_2.fastq -m 200 -t 96 --metaspades -o ASSEMBLY
```
...
```
metawrap2 kraken2 -o KRAKEN -t 96 -s 1000000 CLEAN_READS/ERR*fastq ASSEMBLY/final_assembly.fasta
```
...
```
metawrap2 binning -o INITIAL_BINNING -t 96 -a ASSEMBLY/final_assembly.fasta --metabat2 --maxbin2 --concoct CLEAN_READS/ERR*fastq
```
...
```
metawrap2 bin_refinement -o BIN_REFINEMENT -t 96 -A INITIAL_BINNING/metabat2_bins/ -B INITIAL_BINNING/maxbin2_bins/ -C INITIAL_BINNING/concoct_bins/ -c 50 -x 10
```
...
set; it will likely have low contamination and completion. You will also see `.stats` files for each one
...
The `.stats` files contain useful information about each bin, including its completeness and contamination.
For example, `cat BIN_REFINEMENT/metawrap_50_10.stats`:
```
...
```
metawrap2 blobology -a ASSEMBLY/final_assembly.fasta -t 96 -o BLOBOLOGY --bins BIN_REFINEMENT/metawrap_50_10_bins CLEAN_READS/ERR*fastq
```
...
```
metawrap2 reassemble_bins -o BIN_REASSEMBLY -1 CLEAN_READS/ALL_READS_1.fastq -2 CLEAN_READS/ALL_READS_2.fastq -t 96 -m 800 -c 50 -x 10 -b BIN_REFINEMENT/metawrap_50_10_bins
```
...
Looking at the output in `BIN_REASSEMBLY/reassembled_bins.stats`, we can see which bins were improved
...
We already got an idea of the approximate taxonomy of each bin from CheckM in the `.stats` files in the
...
```
metawrap2 classify_bins -b BIN_REASSEMBLY/reassembled_bins -o BIN_CLASSIFICATION -t 48
```
...
```
metawrap2 annotate_bins -o FUNCT_ANNOT -t 96 -b BIN_REASSEMBLY/reassembled_bins/
```
